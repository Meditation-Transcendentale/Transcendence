...
export class ShieldSystem extends System {
	update(entities: Entity[], deltaTime: number): void {
		entities.forEach(entity => {
			if (entity.hasComponent(PaddleComponent) && entity.hasComponent(InputComponent)) {
	...
	private spawnShield(input: InputComponent, player: PaddleComponent, shield: ShieldComponent): void {
		...
		if (player.angleFactor != 0)
			player.shieldIsActivate = input.down;
	
		if (input.down === true) {
			player.lastInputDelay = performance.now();
			player.angleFactor = Math.max(0, player.angleFactor - 0.01);
		} else if (performance.now() - player.lastInputDelay >= 500) {
			player.angleFactor = Math.min(0.5, player.angleFactor + 0.01);
		}
		if (player.angleFactor == 0)
			player.shieldIsActivate = false;
		if (player.oldAngleFactor != player.angleFactor)
		{
			player.angle = Math.PI * 0.5 * player.angleFactor;
			applyVertexRotation(player);
		}
		player.oldAngleFactor = player.angleFactor;
	}
