import { Engine, Scene, Vector3, Vector2, ArcRotateCamera, HemisphericLight, MeshBuilder, StandardMaterial, Mesh, PolygonMeshBuilder } from "@babylonjs/core";
...
class Game {
	...
	constructor(canvas: HTMLCanvasElement) {
		...
		const player = new Player(this.scene, new Vector3(0, 0, 0));
		...
		this.engine.runRenderLoop(() => {
			...
			player.update();
			ball.update(delta, player, cols, layers, bricks);
	...
	private createArena() {
		const arena = MeshBuilder.CreateDisc("arena", { radius: 5, tessellation: 128 }, this.scene);
