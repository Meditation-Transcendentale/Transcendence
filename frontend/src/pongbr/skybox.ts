...
export function createBlackHoleBackdrop(scene: Scene, statuePosition: Vector3, pongRoot: TransformNode) {
	// Create a large plane behind the statue
	...
	// Position it behind the statue
	backdrop.position = new Vector3(
		statuePosition.x - 2000,
		statuePosition.y,
		statuePosition.z  // Move it back behind statue
	);

	// Rotate to face the camera/arena
...
export function createSkybox(scene: Scene) {
	const blackholeSkybox = MeshBuilder.CreateSphere("blackholeSkybox", { diameter: 5000 }, scene);
	const blackholeMaterial = new ShaderMaterial("blackholeMaterial", scene, {
		vertex: "blackhole",
		fragment: "blackhole"
	}, {
	...
	blackholeMaterial.setFloat("time", 0);
	blackholeMaterial.setVector2("resolution", new Vector3(1024, 1024)); // Note: using Vector3 for Vector2
	blackholeMaterial.backFaceCulling = false;
	blackholeSkybox.material = blackholeMaterial;
	blackholeSkybox.infiniteDistance = true;

	// Animate the fractal black hole
	scene.registerBeforeRender(() => {
		blackholeMaterial.setFloat("time", performance.now() * 0.001);
	});
