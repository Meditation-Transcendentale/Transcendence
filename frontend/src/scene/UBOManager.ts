...
export class UBOManager {
  ...
  constructor(assets: Assets) {
    ...
    this.assets.fogUBO.updateMatrix(
      "projection",
      this.assets.camera.getProjectionMatrix()
    );
    this.assets.fogUBO.updateMatrix(
      "iprojection",
      this.assets.camera.getProjectionMatrix().invertToRef(this.tempMatrix)
    );
    ...
    this.assets.fogUBO.updateFloat(
      "spotIntensity",
      this.assets.flashLight.intensity
    );
    this.assets.fogUBO.updateFloat("spotRange", this.assets.flashLight.range);
    this.assets.fogUBO.updateFloat(
      "spotAngle",
      Math.cos(this.assets.flashLight.angle)
    );
    this.assets.fogUBO.updateFloat("spotExp", this.assets.flashLight.exponent);
    this.assets.fogUBO.updateFloat(
      "pointAIntensity",
      this.assets.ballLight.intensity
    );
    this.assets.fogUBO.updateFloat("pointARange", this.assets.ballLight.range);
    this.assets.fogUBO.updateFloat(
      "pointBIntensity",
      this.assets.cubeLight.intensity
    );
    this.assets.fogUBO.updateFloat("pointBRange", this.assets.cubeLight.range);
    this.assets.fogUBO.updateColor3(
      "spotColor",
      this.assets.flashLight.diffuse
    );
    this.assets.fogUBO.updateVector3(
      "spotPosition",
      this.assets.flashLight.position
    );
    this.assets.fogUBO.updateVector3(
      "spotDirection",
      this.assets.flashLight.direction
    );
    this.assets.fogUBO.updateColor3(
      "pointAColor",
      this.assets.ballLight.diffuse
    );
    this.assets.fogUBO.updateVector3(
      "pointAPosition",
      this.assets.ballLight.position
    );
    this.assets.fogUBO.updateColor3(
      "pointBColor",
      this.assets.cubeLight.diffuse
    );
    this.assets.fogUBO.updateVector3(
      "pointBPosition",
      this.assets.cubeLight.position
    );
  }
  ...
  public update() {
    ...
    this.assets.fogUBO.updateMatrix(
      "iview",
      sceneManager.camera.getViewMatrix().invertToRef(this.tempMatrix)
    );
    this.assets.fogUBO.updateMatrix(
      "world",
      sceneManager.camera.worldMatrixFromCache
    );
    ...
    this.assets.fogUBO.updateVector3(
      "pointAPosition",
      this.assets.ballMesh.position
        .addToRef(this.assets.ballRoot.position, this.tempVector3)
        .scaleInPlace(this.assets.ballRoot.scalingDeterminant)
    );
    this.assets.fogUBO.updateVector3(
      "pointBPosition",
      this.assets.cubeMesh.position
        .addToRef(this.assets.monolithRoot.position, this.tempVector3)
        .scaleInPlace(this.assets.monolithRoot.scalingDeterminant)
    );
    this.assets.fogUBO.updateFloat(
      "pointBIntensity",
      this.assets.cubeLight.isEnabled() ? this.assets.cubeLight.intensity : 0
    );
  ...
  public resize() {
    this.assets.fogUBO.updateMatrix(
      "projection",
      sceneManager.camera.getProjectionMatrix()
    );
    this.assets.fogUBO.updateMatrix(
      "iprojection",
      sceneManager.camera.getProjectionMatrix().invertToRef(this.tempMatrix)
    );
  }
}
