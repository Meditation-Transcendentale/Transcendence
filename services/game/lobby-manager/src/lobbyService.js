...
class Lobby {
	constructor({ id, mode, map, size }) {
		...
		if (mode == "tournament")
			this.maxPlayers = Number(this.tournamentSize);
		else
	...
	allReadyBr() {
		return (
			this.mode == "br" && [...this.players.values()].every((p) => p.isReady)
		);
	}
...
export default class LobbyService {
	...
	quit(lobbyId, userId) {
		...
		natsClient.publish(
			`notification.${userId}.status`,
			encodeStatusUpdate({ sender: userId, status: "online" })
		);
		if (isEmpty) {
		...
		if (lobby.allReady() || lobby.allReadyBr()) {
			if (lobby.mode == `tournament`) {
				...
				try {
					...
					const respTournament =
						decodeTournamentCreateResponse(replyBufTournament);
	...
	cleanup() {
		...
		for (const [id, lobby] of this.lobbies) {
			if (lobby.isEmpty()) {
				...
				console.log(
					`Cleaning up stale lobby: ${id} (created: ${new Date(
						lobby.createdAt
					)}, last activity: ${new Date(lobby.lastActivity)})`
				);
