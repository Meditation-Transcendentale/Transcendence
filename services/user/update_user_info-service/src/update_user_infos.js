...
const verifyApiKey = (req, res, done) => {
  ...
  if (apiKey !== process.env.API_GATEWAY_KEY) {
    return res
      ...
      .send({ message: returnMessages.UNAUTHORIZED });
  }
  done();
};
...
async function checkPassword2FA(user, password, token) {
  ...
  if (!isPasswordValid) {
    throw {
      status: userReturn.USER_022.http,
      code: userReturn.USER_022.code,
      message: userReturn.USER_022.message,
    };
  }
  if (user.two_fa_enabled == true) {
    if (!token) {
      throw {
        status: userReturn.USER_023.http,
        code: userReturn.USER_023.code,
        message: userReturn.USER_023.message,
      };
    }

    try {
      const response = await axios.post(
        "https://update_user_info-service:4003/verify-2fa",
        { token },
        {
          headers: {
            user: JSON.stringify({ uuid: user.uuid }),
...
async function getAvatarCdnUrl(avatar, uuid) {
  ...
  if (!fileType || !fileType.mime.startsWith("image/")) {
    throw {
      status: userReturn.USER_012.http,
      code: userReturn.USER_012.code,
      message: userReturn.USER_012.message,
    };
  }
...
app.patch(
  ...
  handleErrors(async (req, res) => {
    const user = await natsRequest(nats, jc, "user.getUserFromHeader", {
      headers: req.headers,
    });

    if (!req.body) {
      throw {
        status: userReturn.USER_021.http,
        code: userReturn.USER_021.code,
        message: userReturn.USER_021.message,
      };
    }
    ...
    if (username && USERNAME_REGEX.test(username) === false) {
      throw {
        status: userReturn.USER_010.http,
        code: userReturn.USER_010.code,
        message: userReturn.USER_010.message,
      };
    } else {
      await checkPassword2FA(user, password, token);
      await natsRequest(nats, jc, "user.updateUsername", {
        username,
        userId: user.id,
      });
    }
    try {
      nats.publish(
        `notification.${user.id}.status`,
        encodeStatusUpdate({ sender: user.id, status: "update" })
      );
    } catch (err) {
      console.log (err);
    }
    res
      ...
      .send({ message: returnMessages.USERNAME_UPDATED });
  })
);
...
app.patch(
  ...
  handleErrors(async (req, res) => {
    ...
    if (!avatar) {
      throw {
        status: userReturn.USER_007.http,
        code: userReturn.USER_007.code,
        message: userReturn.USER_007.message,
      };
    }
    ...
    await natsRequest(nats, jc, "user.updateAvatar", {
      avatar: cdnPath,
      userId: user.id,
    });
    // try {
    ...
    res
      .code(statusCode.SUCCESS)
      .send({ message: returnMessages.AVATAR_UPDATED, data: { cdnPath } });
  })
);
...
app.patch(
  ...
  handleErrors(async (req, res) => {
    ...
    if (!password) {
      throw {
        status: userReturn.USER_005.http,
        code: userReturn.USER_005.code,
        message: userReturn.USER_005.message,
      };
    }

    if (!newPassword) {
      throw {
        status: userReturn.USER_006.http,
        code: userReturn.USER_006.code,
        message: userReturn.USER_006.message,
      };
    }

    if (PASSWORD_REGEX.test(newPassword) === false) {
      throw {
        status: userReturn.USER_009.http,
        code: userReturn.USER_009.code,
        message: userReturn.USER_009.message,
      };
    }
    ...
    await natsRequest(nats, jc, "user.updatePassword", {
      hashedPassword,
      userId: user.id,
    });
    ...
    res
      .code(statusCode.SUCCESS)
      .send({ message: returnMessages.PASSWORD_UPDATED });
  })
);
