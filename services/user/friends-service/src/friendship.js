...
async function checkFriendshipStatus(user, friend) {
	...
	if (friendship) {
		if (friendship.status === 'accepted') {
			throw { status : friendshipReturn.FRIEND_012.code, message: friendshipReturn.FRIEND_012.message };
		} else if (friendship.status === 'pending' && friendship.user_id_1 === user.id) {
			throw { status : friendshipReturn.FRIEND_013.code, message: friendshipReturn.FRIEND_013.message };
		} else if (friendship.status === 'pending' && friendship.user_id_2 === user.id) {
			throw { status : friendshipReturn.FRIEND_014.code, message: friendshipReturn.FRIEND_014.message };
		}
	}
	if (isBlocked) {
		throw { status : friendshipReturn.FRIEND_018.code, message: friendshipReturn.FRIEND_018.message };
	}
	if (isBlockedBy) {
		throw { status : friendshipReturn.FRIEND_016.code, message: friendshipReturn.FRIEND_016.message };
	}
}
...
app.post('/add', handleErrors(async (req, res) => {
	...
	if (!addedPlayerUsername) {
		throw { status : userReturn.USER_004.code, message: userReturn.USER_004.message };
	}

	const friend = await natsRequest(nats, jc, 'user.getUserFromUsername', { username: addedPlayerUsername });

	if (user.id === friend.id) {
		throw { status : friendshipReturn.FRIEND_006.code, message: friendshipReturn.FRIEND_006.message };
	}
...
app.post('/accept', handleErrors(async (req, res) => {
	...
	if (friendship.status !== 'pending') {
		throw { status : friendshipReturn.FRIEND_002.code, message: friendshipReturn.FRIEND_002.message };
	}
...
app.delete('/decline', handleErrors(async (req, res) => {
	...
	if (!requestFrom) {
		...
		throw { status : friendshipReturn.FRIEND_008.code, message: friendshipReturn.FRIEND_008.message };
	}
...
app.delete('/delete', handleErrors(async (req, res) => {
	...
	if (!friendName) {
		...
		throw { status : friendshipReturn.FRIEND_007.code, message: friendshipReturn.FRIEND_007.message };
	}
	...
	if (!friendship || friendship.status !== 'accepted') {
		throw { status : friendshipReturn.FRIEND_001.code, message: friendshipReturn.FRIEND_001.message };
	}
...
app.post('/block', handleErrors(async (req, res) => {
	...
	if (user.id === blockedUser.id) {
		throw { status : friendshipReturn.FRIEND_009.code, message: friendshipReturn.FRIEND_009.message };
	}

	const isBlocked = await natsRequest(nats, jc, 'user.isBlocked', { userId: user.id, blockedUserId: blockedUser.id });
	if (isBlocked) {
		throw { status : friendshipReturn.FRIEND_015.code, message: friendshipReturn.FRIEND_015.message };
	}
...
app.delete('/unblock', handleErrors(async (req, res) => {
	...
	const blockedUser = await natsRequest(nats, jc, 'user.getUserFromUsername', { username: blockedUserName });
	if (user.id === blockedUser.id) {
		throw { status : friendshipReturn.FRIEND_010.code, message: friendshipReturn.FRIEND_010.message };
	}

	const isBlocked = await natsRequest(nats, jc, 'user.isBlocked', { userId: user.id, blockedUserId: blockedUser.id });
	if (!isBlocked) {
		throw { status : friendshipReturn.FRIEND_017.code, message: friendshipReturn.FRIEND_017.message };
	}
