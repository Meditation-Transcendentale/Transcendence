...
app.post('/login', { schema: loginSchema }, handleErrors(async (req, res) => {
	...
	if (user.two_fa_enabled == true) {
		...
		try {
			const response = await axios.post('https://update_user_info-service:4003/verify-2fa', { token, userUUID: user.uuid }, { headers: {'user': JSON.stringify({ id: user.id }), 'x-api-key': process.env.API_GATEWAY_KEY } , httpsAgent: agent });
